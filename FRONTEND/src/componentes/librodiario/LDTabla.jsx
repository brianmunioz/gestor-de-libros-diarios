...
const LDTabla = ({ datos, id, editar }) => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("acientos_" + id))
      setNuevosArreglos(JSON.parse(sessionStorage.getItem("acientos_" + id)));
  }, []);
  const agregarAciento = () => {
    ...
    if (sessionStorage.getItem("acientos_" + id))
      arregloDeAcientos.push(
        ...JSON.parse(sessionStorage.getItem("acientos_" + id))
      );
  ...
  return (
    <Table bordered hover>
      ...
      <tbody>
        ...
        {nuevosArreglos.length > 0 &&
          sessionStorage.getItem("acientos_" + id) &&
          nuevosArreglos.map((aciento) => {
            return <Aciento aciento={aciento} color={"rgb(169 164 159)"} />;
          })}
        {sessionStorage.getItem("operacion_" + id) && (
          <Aciento
            aciento={{
              descripcion_operacion:
                "Nueva operación: " + sessionStorage.getItem("operacion_" + id),
            }}
            colorOperacion={"rgb(143 143 18)"}
          />
        )}
        {editar && (
          <tr>
            ...
            <td>
              <Button variant="outline-dark" onClick={agregarAciento}>
                Add
              </Button>
            </td>
          </tr>
        ...
        {error && (
          <tr colspan="8">
            <td colspan="8" className="text-center bg-danger text-light p-2">
              {error}
            </td>
          </tr>
        )}
